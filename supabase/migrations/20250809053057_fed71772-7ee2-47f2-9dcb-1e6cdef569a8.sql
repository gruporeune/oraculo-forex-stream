...
INSERT INTO user_plans (user_id, plan_name, is_active, purchase_date)
VALUES (
  (SELECT id FROM profiles WHERE full_name = 'anavic123'),
  'master',
  true,
  now()
);

-- Activate magalhaes123 with partner plan
UPDATE profiles 
SET plan = 'partner'
WHERE full_name = 'magalhaes123';

INSERT INTO user_plans (user_id, plan_name, is_active, purchase_date)
VALUES (
  (SELECT id FROM profiles WHERE full_name = 'magalhaes123'),
  'partner',
  true,
  now()
);